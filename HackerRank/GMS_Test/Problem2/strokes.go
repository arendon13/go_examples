...
func main() {
	...
	pictureCount, err := strconv.ParseInt(strings.TrimSpace(readLine(reader)), 10, 64)
	checkError(err)

	var picture []string

	for i := 0; i < int(pictureCount); i++ {
		pictureItem := readLine(reader)
		picture = append(picture, pictureItem)
	}
