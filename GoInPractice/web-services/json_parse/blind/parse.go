...
func main() {
	...
	edu := m["education"].([]interface{})
	for _, el := range edu {
		edMap := el.(map[string]interface{})
		fmt.Println(edMap["institution"])
		fmt.Println(edMap["degree"])
	}
	// m2 := edu[0].(map[string]interface{})
	// fmt.Println(m2["institution"])
